...
class Absolutedesignlimited_Commercebug_Helper_Formatlog_Allsimple extends Absolutedesignlimited_Commercebug_Helper_Data
{
        ...
        if(isset($object->observers))
        {
            foreach($object->observers as $item)
            {
                foreach(get_object_vars($item) as $name=>$props)
                {
                    ...
                    #Absolutedesignlimited_Commercebug_Model_Shim::Log('Mage2 refactoring needed at ' . __CLASS__ . ' ' . __LINE__);
                    unset($info['config']);
                    $result .= implode("\t", $info) . "\n";
                }                
    ...
    public function formatLayout($object)
    {
        ...
        $this->_info[] = 'Graphviz: ';
        $this->_info[] = Absolutedesignlimited_Commercebug_Model_Observer_Dot::renderGraph();
        return $this;			
    }
