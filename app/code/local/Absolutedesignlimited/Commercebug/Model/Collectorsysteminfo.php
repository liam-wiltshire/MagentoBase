...
*/
class Absolutedesignlimited_Commercebug_Model_Collectorsysteminfo extends Absolutedesignlimited_Commercebug_Model_Observingcollector
{
    protected $_items;
    public function collectInformation($observer)
    {
        ...
        {
            Absolutedesignlimited_Commercebug_Model_Crossareaajax::validateControllers();
        }
        
        $system_info->ajax_paths = new stdClass;
        $system_info->form_key = $this->getShim()->getSingleton('core/session')->getFormKey();
        foreach(Absolutedesignlimited_Commercebug_Model_Crossareaajax::getAjaxActionNames() as $action)
        {
    ...
    public function createAjaxUrl($action)
    {
        ...
        #Absolutedesignlimited_Commercebug_Model_Shim::Log('Mage2 Cleanup in ' . __CLASS__ . ' ' . __LINE__);
